...
class WindowPlayer :  AbBaseUIPlayer, View.OnClickListener, PointSeekBar.PointDrag,
    ...
    private var mPlayType: SuperPlayerDef.PlayerType? = null// 当前播放视频类型
    private var mCurrentPlayState: SuperPlayerDef.PlayerState? = SuperPlayerDef.PlayerState.END // 当前播放状态
    ...
    fun isLive() = (mPlayType == SuperPlayerDef.PlayerType.LIVE || mPlayType == SuperPlayerDef.PlayerType.LIVE_SHIFT)
    ...
    private fun togglePlayState() {
        when (mCurrentPlayState) {
            SuperPlayerDef.PlayerState.PAUSE, SuperPlayerDef.PlayerState.END -> if (mControllerCallback != null) {
                mControllerCallback!!.onResume()
            }
            SuperPlayerDef.PlayerState.PLAYING, SuperPlayerDef.PlayerState.LOADING -> {
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        if ((event?.action == MotionEvent.ACTION_CANCEL || event?.action == MotionEvent.ACTION_UP) && mVideoGestureDetector != null && mVideoGestureDetector!!.isVideoProgressModel) {
            ...
            seekTime = if (mPlayType == SuperPlayerDef.PlayerType.LIVE || mPlayType == SuperPlayerDef.PlayerType.LIVE_SHIFT) {
    ...
    override val playerMode: SuperPlayerDef.PlayerMode
        get() = SuperPlayerDef.PlayerMode.WINDOW
