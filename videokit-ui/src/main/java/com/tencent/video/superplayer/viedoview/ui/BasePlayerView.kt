...
import com.tencent.video.superplayer.casehelper.PlayKeyListConfig
import com.tencent.video.superplayer.casehelper.onNext
import com.tencent.video.superplayer.kit.PlayerKit.checkOp
import com.tencent.video.superplayer.viedoview.AbBaseUIPlayer
...
import me.shetj.sdk.video.player.ITXSnapshotListener
 ...
 */
open class BasePlayerView<T : View> : FrameLayout, TimerConfigure.CallBack, SuperPlayer<T>,
    UIPlayer,
    ...
    protected lateinit var mSuperPlayer: SuperPlayer<T>
    ...
    // 悬浮窗模式控制view
    protected var mFloatPlayer: FloatPlayer<T>? = null

    // 腾讯云视频播放view
    protected var mTXCloudVideoView: VideoPlayerView<T>? = null
    ...
    fun setPlayer(player: SuperPlayer<T>) {
        mSuperPlayer = player
        setObserver(this)
    }
    ...
    override fun setPlayerView(videoView: VideoPlayerView<T>) {
    ...
    override fun setUICallback(callback: me.shetj.sdk.video.UIPlayer.VideoViewCallback?) {
    ...
    override fun onPlayTimeShiftLive(player: SuperPlayer<T>?, url: String?) {
    }
    ...
    private val mControllerCallback: me.shetj.sdk.video.UIPlayer.VideoViewCallback =
        object : me.shetj.sdk.video.UIPlayer.VideoViewCallback {
            ...
            override fun onSnapshot() {
                mSuperPlayer.snapshot(object : ITXSnapshotListener {
    ...
    open fun processStopFloatWindow() {
        ...
        if (viewContext is Activity && !viewContext.isDestroyed) {
            ...
            mSuperPlayer.pause()
            mTXCloudVideoView?.let { mSuperPlayer.setPlayerView(it) }
            mSuperPlayer.resume()
