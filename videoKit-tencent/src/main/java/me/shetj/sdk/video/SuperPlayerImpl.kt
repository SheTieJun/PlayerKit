...
internal class SuperPlayerImpl(
        ...
        when (event) {
            ...
            PLAY_ERR_NET_DISCONNECT, PLAY_EVT_PLAY_END -> if (playerType == PlayerType.LIVE_SHIFT) {  // 直播时移失败，返回直播
                ...
                if (event == PLAY_ERR_NET_DISCONNECT) {
                    onError(me.shetj.sdk.video.SuperPlayerCode.NET_ERROR, "网络不给力,点击重试")
                } else {
                    onError(
                        me.shetj.sdk.video.SuperPlayerCode.LIVE_PLAY_END,
    ...
    override fun onPlayEvent(player: TXVodPlayer, event: Int, param: Bundle) {
        ...
        if (event < 0) { // 播放点播文件失败
            ...
            onError(
                me.shetj.sdk.video.SuperPlayerCode.VOD_PLAY_FAIL,
                param.getString(EVT_DESCRIPTION)
            )
        }
    }
    ...
    private fun playWithModel(model: VideoPlayerModel?) {
        ...
        if (model.videoId != null) { // 根据FileId播放
            mCurrentProtocol!!.sendRequest(object : IPlayInfoRequestCallback {
                ...
                override fun onError(errCode: Int, message: String) {
                    ...
                    this@SuperPlayerImpl.onError(
                        me.shetj.sdk.video.SuperPlayerCode.VOD_REQUEST_FILE_ID_FAIL,
            ...
            if (TextUtils.isEmpty(videoURL)) {
                onError(
                    me.shetj.sdk.video.SuperPlayerCode.PLAY_URL_EMPTY,
                    "播放视频失败，播放链接为空"
                )
                return
            }
    ...
    override fun setPlayerView(videoView: VideoPlayerView<TXCloudVideoView>) {
        if (playerType == PlayerType.VOD) {
            mVodPlayer!!.setPlayerView(videoView.getPlayerView())
        } else {
            mLivePlayer!!.setPlayerView(videoView.getPlayerView())
        }
    }
